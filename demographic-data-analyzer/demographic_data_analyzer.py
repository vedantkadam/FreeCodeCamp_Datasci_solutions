...
def calculate_demographic_data(print_data=True):
    ...
    total_num = len(df)
    percentage_bachelors = round(bach / total_num * 100, 1)
    ...
    non_per_hi = len(higher_education[higher_education.salary == ">50K"])
    higher_education_rich = round(non_per_hi / len(higher_education) * 100, 1)
    non_per_lo = len(lower_education[lower_education.salary == ">50K"])
    lower_education_rich = round(non_per_lo / len(lower_education) * 100, 1)
    ...
    people_of_in=df[df['native-country']=='India']
    occupation_counts=people_of_in['occupation'].value_counts()
